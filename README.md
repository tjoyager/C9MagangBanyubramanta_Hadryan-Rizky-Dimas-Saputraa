...
ros2 run vision_ov_yolo yolo_detector --ros-args \
    -p model_path:="/home/user/ros2_ws/src/vision_ov_yolo/models/waras.onnx" \
    -p video_path:="/home/user/ros2_ws/src/vision_ov_yolo/data/sample.mp4"
...
# 2. Jalankan Bridge (Contoh port custom)
...
Proyek ini terdiri dari satu paket utama: submarine_sim. Paket ini memuat deskripsi robot, konfigurasi dunia (World), dan file peluncuran (Launch files) untuk memunculkan robot yang dilengkapi kamera dan sistem kontrol gerak, serta integrasi dengan sistem visi komputer.
...
Di menu atas Gazebo, klik icon Cylinder (Tabung) atau Sphere (Bola).

Klik di lantai simulasi (pastikan berada di jangkauan kamera robot).

Klik kanan pada objek tersebut -> Edit Model.

Pilih tab Material (atau Link Inspector -> Visual -> Material).

Ubah warna menjadi Orange (untuk simulasi Flare) atau warna lain sesuai model YOLO Anda.
...
ros2 run vision_ov_yolo yolo_detector --ros-args \
    -p model_path:="/home/hadryan/ros2_ws/src/vision_ov_yolo/models/best.onnx" \
